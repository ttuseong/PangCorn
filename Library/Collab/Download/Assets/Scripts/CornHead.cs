...
public class CornHead : MonoBehaviour {
    ...
    private void Update()
    {
        if (Corn.transform.GetChild(0).transform.GetChild(0).GetComponent<BoxCollider2D>().enabled == false && !theGM.Allstop)
        {
            Corn.transform.GetChild(0).transform.GetChild(0).GetComponent<BoxCollider2D>().enabled = true;
        }
